...
/**
 * Milvus 向量存储占位与本地联调实现：保留 host/port 等配置项，数据路径与 {@link InMemoryVectorStore} 一致（进程内 Map）。
 * 接入真实 Milvus 集群时，可在此类中替换为 milvus-sdk-java 的 insert/search 调用。
 */
@Component
public class MilvusVectorStore implements VectorStore {
    ...
    @Value("${vector.milvus.token:}")
    private String token;

    @Value("${vector.milvus.database:default}")
    private String database;

    private final Map<String, Map<String, VectorEntry>> collections = new ConcurrentHashMap<>();
    private final EmbeddingModel embeddingModel;

    public MilvusVectorStore(EmbeddingModel embeddingModel) {
    ...
    @SuppressWarnings("unused")
    public String getConfiguredDatabase() {
        return database;
    }
