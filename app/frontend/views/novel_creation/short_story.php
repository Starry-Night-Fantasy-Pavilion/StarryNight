<div class="page-header"><div class="container"><h1>短篇创作</h1><p>专业的短篇小说创作模式，精炼故事</p></div></div>

<div class="container">
    <form method="POST" action="/novel_creation/do_short_story" class="analysis-form">
        <div class="form-section">
            <h3>创作信息</h3>
            <div class="form-group">
                <label>题材类型 <span class="required">*</span></label>
                <input type="text" name="genre" placeholder="如：言情、悬疑、科幻..." required value="<?= htmlspecialchars($_POST['genre'] ?? '') ?>">
            </div>
            <div class="form-group">
                <label>核心主题 <span class="required">*</span></label>
                <input type="text" name="theme" placeholder="故事的核心主题" required value="<?= htmlspecialchars($_POST['theme'] ?? '') ?>">
            </div>
            <div class="form-group">
                <label>主要人物</label>
            ...
            <div class="form-group">
                <label>情节梗概 <span class="required">*</span></label>
                <textarea name="plot" rows="4" placeholder="简要描述故事的情节发展..." required><?= htmlspecialchars($_POST['plot'] ?? '') ?></textarea>
            </div>
            <div class="form-group">
                <label>目标字数</label>
                <input type="number" name="word_count" value="<?= htmlspecialchars($_POST['word_count'] ?? 2000) ?>" min="1000" max="5000">
            </div>
        </div>
